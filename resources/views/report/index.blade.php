...
<body>
    ...
    @foreach ($practices as $prac)
        @if (($prac->is_avvio_progettazione && substr($prac->codice, 1, 2) == substr($annoBase, 2, 2)) || ($prac->ese_at > Date($annoBase . "-01-01") && Date($prac->ese_at) < Date($annoBase . "-12-31")))
            @php
    ...
    @foreach ($practices as $prac)
        @if (($prac->is_lavori_in_corso && $prac->consegna_lavori_at > Date($annoBase . "-01-01") && $prac->consegna_lavori_at < Date($annoBase . "-12-31")))
            @php
    ...
    <div class="flex justify-end">
        ...
        <a href="{{ route('practices.index') }}?is_in_corso=on" class="m-2 p-2 border border-blue-600 rounded-2xl">Vai a
            Area Riservata</a>
    </div>
    <br>
    <span>
